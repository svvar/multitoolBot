...
# WORKS (31.10.2024)
# TikTok likes to change html structure, so this function may need fixes in the future
def save_tiktok_video(video_url, download_folder):
    driver = webdriver.Chrome(options=options)
    driver.get(video_url)
    video = driver.find_element(By.TAG_NAME, 'video')
    source = video.find_elements(By.TAG_NAME, 'source')
    src = ''
    for s in source:
        if 'webapp-prime' in s.get_attribute('src'):
