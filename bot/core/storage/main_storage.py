...
from sqlalchemy import Column, Integer, Text, DateTime, func, JSON, Sequence, Row
...
class MailingStorage(Base):
    __tablename__ = 'mailing_storage'

    id = Column(Integer, primary_key=True, autoincrement=True)
    mailing_name = Column(Text)
    chat_data = Column(JSON)                                        # key - tg_id, value - message id
...
async def save_mailing_results(mailing_name: str, chat_data: dict):
    async with sessionmaker() as session:
        await session.execute(
            sa.insert(MailingStorage).values(mailing_name=mailing_name, chat_data=chat_data)
        )
        await session.commit()


async def get_mailings() -> list[Row]:
    async with sessionmaker() as session:
        mailings = await session.execute(
            sa.select(MailingStorage.id, MailingStorage.mailing_name)
            .order_by(MailingStorage.id.desc())
        )
        return list(mailings.all())


async def get_mailing_data(mailing_id: int) -> dict:
    async with sessionmaker() as session:
        mailing = await session.execute(
            sa.select(MailingStorage.chat_data).where(MailingStorage.id == mailing_id)
        )
        return mailing.scalar()
